...
class HologresUtil extends DBEngineUtil {
    ...
    def getValidFieldNames(mysqlConfig: DBConfig, internalConfig: InternalConfig)(implicit crashOnInvalidType: Boolean): String = {
        val conn = getConnection(mysqlConfig)
        val tableDetails = getTableDetails(mysqlConfig, internalConfig)(crashOnInvalidType)
        conn.close()
